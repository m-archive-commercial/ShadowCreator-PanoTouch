...
class HonghuJoystickBeanHold extends HonghuKeyBean {
    private final static String TAG = "HonghuJoystickBean";
    ...
    HonghuJoystickBeanHold(Context context, String spName, String s, int code, int id, int x, int y, String icon, boolean reset) {
        super(context, spName, s, code, id, x, y, icon, reset);
        mProperties[0].id = id;
        mProperties[0].toolType = MotionEvent.TOOL_TYPE_FINGER;
        Log.v(TAG, String.format("JoystickBean of [%s] 模拟按下：(%d, %d)…", s, x, y));
        dispatchMotionEvent(7, 7, MotionEvent.ACTION_DOWN);
...
//            if(!isFinal) {
//                isFinal = true;
                dispatchMotionEvent(x, y, MotionEvent.ACTION_MOVE);
//            }
        } else {
            int N = 5;
            for (int i = 1; i <= N; i++) {
