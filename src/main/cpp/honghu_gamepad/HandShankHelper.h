...
#include "../../../../../../../Library/Android/sdk/ndk/21.4.7075529/toolchains/llvm/prebuilt/darwin-x86_64/sysroot/usr/include/sched.h"
#include "../../../../../../../Library/Android/sdk/ndk/21.4.7075529/toolchains/llvm/prebuilt/darwin-x86_64/sysroot/usr/include/dlfcn.h"
#include "../../../../../../../Library/Android/sdk/ndk/21.4.7075529/toolchains/llvm/prebuilt/darwin-x86_64/sysroot/usr/include/c++/v1/stdlib.h"
...
typedef struct handshank_client_ops {
    ...
    void
    (*HandShank_registerKeyTouchEventCallback)(void **, std::function<void(int, bool, uint8_t)> c);
...
static inline handshank_client_helper_t *HandShankClient_Create() {
    ...
    if (!me->ops) {
        __android_log_print(ANDROID_LOG_ERROR, "HandShankClient", "HandShankClient dlopen error=%s",
                            dlerror());
    ...
    HandShank_initContext initContext = (HandShank_initContext) dlsym(me->libHandle,
                                                                      "HandShank_InitContext");
    if (!initContext) {
        __android_log_print(ANDROID_LOG_ERROR, "HandShankClient",
                            "HandShankClient initContext error=%s", dlerror());
    }
    ...
    me->ops->HandShank_deinitcontext = (HandShank_deinit_context_fn) dlsym(me->libHandle,
                                                                           "HandShank_DeInitContext");

    typedef void (*HandShank_registerTouchEventCallback_fn)(void **, std::function<void(int, int,
                                                                                        uint8_t)> c);
    me->ops->HandShank_registerTouchEventCallback = (HandShank_registerTouchEventCallback_fn) dlsym(
            me->libHandle,
            "HandShank_RegisterTouchEventCallback");

    typedef void (*HandShank_registerHallEventCallback_fn)(void **, std::function<void(int, int,
                                                                                       uint8_t)> c);
    me->ops->HandShank_registerHallEventCallback = (HandShank_registerHallEventCallback_fn) dlsym(
            me->libHandle,
            "HandShank_RegisterHallEventCallback");

    typedef void (*HandShank_registerKeyEventCallback_fn)(void **,
                                                          std::function<void(int, int, uint8_t)> c);
    me->ops->HandShank_registerKeyEventCallback = (HandShank_registerKeyEventCallback_fn) dlsym(
            me->libHandle,
            "HandShank_RegisterKeyEventCallback");

    typedef void (*HandShank_registerKeyTouchEventCallback_fn)(void **,
                                                               std::function<void(int, bool,
                                                                                  uint8_t)> c);
    me->ops->HandShank_registerKeyTouchEventCallback = (HandShank_registerKeyTouchEventCallback_fn) dlsym(
            me->libHandle,
            "HandShank_RegisterKeyTouchEventCallback");

    typedef void (*HandShank_registerChargingEventCallback_fn)(void **, std::function<void(bool,
                                                                                           uint8_t)> c);
    me->ops->HandShank_registerChargingEventCallback = (HandShank_registerChargingEventCallback_fn) dlsym(
            me->libHandle,
            "HandShank_RegisterChargingEventCallback");

    typedef void (*HandShank_registerBatteryEventCallback_fn)(void **,
                                                              std::function<void(int, uint8_t)> c);
    me->ops->HandShank_registerBatteryEventCallback = (HandShank_registerBatteryEventCallback_fn) dlsym(
            me->libHandle,
            "HandShank_RegisterBatteryEventCallback");

    typedef void (*HandShank_registerConnectEventCallback_fn)(void **,
                                                              std::function<void(bool, uint8_t)> c);
    me->ops->HandShank_registerConnectEventCallback = (HandShank_registerConnectEventCallback_fn) dlsym(
            me->libHandle,
            "HandShank_RegisterConnectEventCallback");

    typedef uint8_t (*HandShank_getVersion_fn)(void **);
    me->ops->HandShank_getVersion = (HandShank_getVersion_fn) dlsym(me->libHandle,
                                                                    "HandShank_GetVersion");
    ...
    me->ops->HandShank_getBattery = (HandShank_getBattery_fn) dlsym(me->libHandle,
                                                                    "HandShank_GetBattery");

    typedef uint8_t (*HandShank_getCharging_fn)(void **, uint8_t lr);
    me->ops->HandShank_getCharging = (HandShank_getCharging_fn) dlsym(me->libHandle,
                                                                      "HandShank_GetCharging");
    ...
    me->ops->HandShank_clearBond = (HandShank_clearBond_fn) dlsym(me->libHandle,
                                                                  "HandShank_Clearbond");

    typedef void (*HandShank_ledControl_fn)(void **, uint8_t enable);
    me->ops->HandShank_ledControl = (HandShank_ledControl_fn) dlsym(me->libHandle,
                                                                    "HandShank_LedControl");

    typedef void (*HandShank_vibrateControl_fn)(void **, uint8_t value);
    me->ops->HandShank_vibrateControl = (HandShank_vibrateControl_fn) dlsym(me->libHandle,
                                                                            "HandShank_VibrateControl");
    ...
    me->ops->HandShank_system_property_set = (HandShank_system_property_set_fn) dlsym(me->libHandle,
                                                                                      "HandShank_SystemPropertySet");
...
static inline void
HandShankClient_GetImu(handshank_client_helper_t *me, std::vector<HandShank_IMUDATA> &v, int lr) {
    if (!me) return;
...
static inline void
HandShankClient_RegisterTouchEventCB(handshank_client_helper_t *me,
                                     std::function<void(int, int, uint8_t)> c) {
    if (!me) return;
...
static inline void
HandShankClient_RegisterHallEventCB(handshank_client_helper_t *me,
                                    std::function<void(int, int, uint8_t)> c) {
    if (!me) return;
...
static inline void
HandShankClient_RegisterKeyEventCB(handshank_client_helper_t *me,
                                   std::function<void(int, int, uint8_t)> c) {
    if (!me) return;
...
static inline void
HandShankClient_RegisterKeyTouchEventCB(handshank_client_helper_t *me,
                                        std::function<void(int, int, uint8_t)> c) {
    ALOGD("called HandShankClient_RegisterKeyTouchEventCB\n");
    if (!me) return;
    ALOGD("entered HandShankClient_RegisterKeyTouchEventCB\n");
    if (me->ops->HandShank_registerKeyTouchEventCallback) {
        ALOGD("entered internal HandShankClient_RegisterKeyTouchEventCB\n");
        me->ops->HandShank_registerKeyTouchEventCallback(&me->context, c);
    }
}

static inline void
HandShankClient_RegisterChargingEventCallback(handshank_client_helper_t *me,
                                              std::function<void(bool, uint8_t)> c) {
    if (!me) return;
...
static inline void
HandShankClient_RegisterBatteryEventCallback(handshank_client_helper_t *me,
                                             std::function<void(int, uint8_t)> c) {
    if (!me) return;
...
static inline void
HandShankClient_RegisterConnectEventCallback(handshank_client_helper_t *me,
                                             std::function<void(bool, uint8_t)> c) {
    if (!me) return;
...
static inline int
HandShankClient_GetPose(handshank_client_helper_t *me, float *data, double &ts, int lr) {
    if (!me) return -2;
...
static inline void
HandShankClient_SystemPropertySet(handshank_client_helper_t *me, int type, float value) {
    if (!me) return;
